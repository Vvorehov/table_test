...
test('Set new campaign list functionality', async () => {
    renderWithProviders(<CampaignList />)
    window.AddCampaigns(formatCampaignList([{"id":1111111,"name":"Test","startDate":"9/19/2017","endDate":"3/9/2018","budget":100000},]))
    jest.useFakeTimers()
    setTimeout(
        () => {
...
test('AddCampaigns functionality check', async () => {
    renderWithProviders(<CampaignList />)
    window.AddCampaigns(formatCampaignList([
        ...
        {"id":10,"name":"Realbridge123","startDate":"3/5/2018","endDate":"10/2/2017","Budget":505602}]))
    jest.useFakeTimers()
    setTimeout(
        () => {
            const campaignTest = screen.getByText(new RegExp('Realbridge123'))
            expect(campaignTest).toBeInTheDocument();
        }, 0
    )
})
